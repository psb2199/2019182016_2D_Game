...
class Mid_Enemy_Bullet:
    ...
    def __init__(self,x = -100,y=-100,player_x = 0,player_y = 0):
        if Mid_Enemy_Bullet.image == None:
            Mid_Enemy_Bullet.image = load_image('resources\\mid_enemy_Bullet.png')
        self.x, self.y = x,y
    ...
    def update(self):
        ...
        if self.lifetime > 0:
            self.x -= self.velocity * math.sin(self.degree / 180 * math.pi)
            self.y -= self.velocity * math.cos(self.degree / 180 * math.pi)
